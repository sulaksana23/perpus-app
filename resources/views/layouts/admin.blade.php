    ...
    <div class="min-h-screen lg:flex">
        ...
        <aside class="fixed inset-y-0 left-0 z-40 w-64 -translate-x-full border-r border-slate-700/70 bg-gradient-to-b from-slate-900 to-slate-800 p-3 text-slate-100 shadow-2xl transition-transform peer-checked:translate-x-0 lg:static lg:translate-x-0">
            ...
            <nav class="space-y-1 text-sm">
                ...
                <a class="flex items-center rounded-lg px-2.5 py-2 font-semibold transition {{ request()->routeIs('admin.users.index', 'admin.users.create', 'admin.users.edit') ? 'bg-teal-500/25 text-white' : 'text-slate-200 hover:bg-white/10' }}" href="{{ route('admin.users.index') }}"><i class="bi bi-people me-2"></i>Kelola User</a>
                <a class="flex items-center rounded-lg px-2.5 py-2 font-semibold transition {{ request()->routeIs('admin.users.pending') ? 'bg-teal-500/25 text-white' : 'text-slate-200 hover:bg-white/10' }}" href="{{ route('admin.users.pending') }}"><i class="bi bi-person-check me-2"></i>ACC Akun</a>
